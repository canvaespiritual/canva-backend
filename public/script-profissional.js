...
function mostrarPergunta(index) {
  ...
  ${p.codigos.map((c, i) => {
    ...
    const ctx = window.detalhesFrutosContextos || {};

const textoDescricao =
  ctx[codigo]?.profissional ||
  d?.descricao ||
  "";

    const emoji = String(p.opcoes[i] || "").trim().split(" ")[0];

return `<button class="opcao bg-gray-100 hover:bg-blue-100 p-4 rounded text-left h-full w-full flex items-start gap-2" data-codigo="${c}">
  <span class="text-lg">${emoji}</span>
  <span class="text-sm text-gray-700 leading-snug">${textoDescricao}</span>
</button>`;
  }).join('')}
</div>
