...
document.addEventListener("click", async (e) => {
  if (e.target.id === "btn-diagnostico") {
    ...
    try {
      ...
      // 👇 NOVO: captura e normaliza WhatsApp (opcional)
      const telefoneRaw = document.getElementById("telefone")?.value || "";
      let telefone = telefoneRaw.replace(/\D/g, "");   // deixa só dígitos
      if (telefone) {
        if (!telefone.startsWith("55")) telefone = "55" + telefone; // adiciona DDI Brasil
        // valida se veio (opcional): aceita 11–13 dígitos (ex.: 55 + DDD + 9 dígitos = 13)
        if (!/^(\d{11,13})$/.test(telefone)) {
          hideLoader();
          btn.disabled = false;
          btn.style.opacity = "";
          btn.style.cursor = "";
          document.getElementById("mensagem").textContent = "Verifique o WhatsApp (com DDD).";
          return;
        }
      } else {
        telefone = null; // permanece opcional
      }
      ...
      const dados = { nome, email, telefone, token: "geradoNoFuturo", respostas };
      ...
      const resp = await fetch("/api/salvar-quiz", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ session_id, nome, email, telefone, respostas: notas })
      });
      ...
      window.location.href = `/pagar-mini.html?tipo=completo&session_id=${session_id}${refPart}`;
